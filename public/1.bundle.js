...
/***/ function(module, exports, __webpack_require__) {
	...
	var Level = __webpack_require__(248);
	var Header = __webpack_require__(267);
	var HorizontalConnectionLine = __webpack_require__(268);
	...
	var getBosses = function getBosses(callback) {
	  axios.get('http://localhost:3000/villain-hierarchy').then(function (response) {
	    var bosses = response.data.map(function (boss) {
	      boss.level = 1;
	      boss.drilled = false;
	...
	var Landing = React.createClass({
	  ...
	  render: function render() {
	    ...
	    return React.createElement(
	      ...
	      React.createElement(
	        'div',
	        { className: 'organization-tree-container' },
	        React.createElement(
	          'div',
	          { className: 'organization-level-container' },
	          Object.keys(this.props.levels).map(function (levelNum) {
	            var level = _this.props.levels[levelNum];

	            return React.createElement(Level, { villains: level, key: levelNum, levelNum: levelNum });
	          })
	        ),
	        React.createElement(
	          'svg',
	          ...
	          this.props.villainElements.filter(function (villainElement) {
	            return !!villainElement.getAttribute('boss-id');
	          }).map(function (villainElement) {
	            return React.createElement(HorizontalConnectionLine, {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var React = __webpack_require__(1);
	var MemberCard = __webpack_require__(249);
	...
	var Level = React.createClass({
	  ...
	  componentWillMount: function componentWillMount() {
	    if (!this.props.levelColors[this.props.levelNum]) {
	      this.props.setLevelColor(this.props.levelNum, '#' + (Math.random() * 0xFFFFFF << 0).toString(16));
	    }
	  },
	  render: function render() {
	    var _this = this;
	    ...
	    return React.createElement(
	      'div',
	      { className: 'organization-level' },
	      this.props.villains.sort(function (a, b) {
	        var bossIdCompare = a.bossId - b.bossId;
	        var idCompare = a.id - b.id;
	      ...
	      }).map(function (villain) {
	        return React.createElement(MemberCard, _extends({}, villain, { key: villain.id, color: _this.props.levelColors[_this.props.levelNum] }));
	      })
	    );
	  }
	});
...
/***/ function(module, exports, __webpack_require__) {
	...
	*/
	...
	var setTooltip = function setTooltip(title, cardId) {
	...
	var getCardTooltipTitle = function getCardTooltipTitle(drilled) {
	  if (drilled) {
	...
	var getMaxWindowHeight = function getMaxWindowHeight() {
	...
	var MemberCard = React.createClass({
	  ...
	  componentDidMount: function componentDidMount() {
	    var villainElement = document.getElementById(this.props.id);
	    var title = getCardTooltipTitle(this.props.drilled);
	  ...
	  handleMemberCardClickEvent: function handleMemberCardClickEvent(event) {
	    var _this = this;
	    ...
	    axios.get('http://localhost:3000/villain-hierarchy/' + this.props.id).then(function (response) {
	      _this.props.updateVillainDrilledStatus(_this.props.level, _this.props.id);

	      if (response.data.length > 0) {
	        var subordinates = response.data.map(function (subordinate) {
	          subordinate.level = nextLevel;
	          subordinate.drilled = false;
	          subordinate.bossId = _this.props.id;

	          return subordinate;
	        });

	        _this.props.setSubordinates(subordinates, nextLevel);
	      } else {
...
/***/ function(module, exports, __webpack_require__) {
	...
	var areAllDrilled = function areAllDrilled(levels) {
	...
	var discoverAllVillains = function discoverAllVillains(levels, callback) {
	  Object.keys(levels).map(function (levelNum) {
	    levels[levelNum].map(function (villain) {
	      ...
	      axios.get('http://localhost:3000/villain-hierarchy/' + villain.id).then(function (response) {
	        if (response.data.length > 0) {
	          var _levels;
	          ...
	          (_levels = levels[villain.level + 1]).push.apply(_levels, _toConsumableArray(subordinates));
	...
	var Header = React.createClass({
	  displayName: 'Header',
	  handleDiscoverAllClickEvent: function handleDiscoverAllClickEvent(event) {
	    var levels = Object.assign({}, this.props.levels);

	    discoverAllVillains(levels, this.props.setAllLevels);
	  },
	  handleClearAllClickEvent: function handleClearAllClickEvent(event) {
	    this.props.clearLevels();
	  },
	  render: function render() {
	    var buttonDisplay = void 0;
...
/***/ function(module, exports, __webpack_require__) {
	...
	var HorizontalConnectionLine = function HorizontalConnectionLine(props) {
	  var frontBox = document.getElementById(props.frontBoxId);
	  var frontBoxRect = frontBox.getBoundingClientRect();
	  var frontBoxStyle = window.getComputedStyle(frontBox);
	  var frontBoxHorizontalPadding = parseInt(frontBoxStyle.getPropertyValue('padding-left')) + parseInt(frontBoxStyle.getPropertyValue('padding-right'));
	  var backBox = document.getElementById(props.backBoxId);
